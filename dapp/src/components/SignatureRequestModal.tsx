...
export const SignatureRequestModal: React.FC<Props> = ({ ...props }) => {
  const { signer, initSigner, walletConnectURI, setWalletConnectURI, signatureRequestHandler } =
    useContext(SymfoniContext);
      ...
      {requests.length > 0 && (
        ...
        >
          ...
          >
            ...
            {requests.length > 1 && (
              <Text>{`Signatur ${requestProcess + 1} av ${requests.length}`}</Text>
            )}
